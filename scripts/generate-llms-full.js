...
function cleanContent(raw, filePath) {
  ...
  // Strip HTML tags and Nunjucks template tags
  text = text
    .replace(/{%[\s\S]*?%}/g, '')
    .replace(/{{[\s\S]*?}}/g, '')
...
function generate() {
  ...
  console.log(`Processing ${files.length} pages for llms-full.txt...`);
  
  let fullText = `# FinOps LLM — Complete Research & Knowledge Base (llms-full.txt)\n\n`;
  fullText += `Content-Signal: ai-train=no, search=yes, ai-input=yes\n\n`;
  fullText += `This document contains the complete text of all 208 public research articles, benchmarks, and guides on finopsllm.com.\n\n`;
  fullText += `=`.repeat(80) + `\n\n`;
