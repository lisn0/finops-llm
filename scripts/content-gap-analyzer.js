...
// Most articles now live as JSON in _data/articles, not as .njk in src/research.
// Scanning only src/research reported three covered topics as "High Gap".
const dataArticlesDir = path.join(__dirname, '..', 'src', '_data', 'articles');
...
const targetTopicCluster = [
  ...
  { id: 'reasoning-model-cost', topic: 'Reasoning Token Cost Allocation', targetQuery: 'how to track reasoning tokens cost' },
  { id: 'batch-api-finops', topic: 'Batch API vs Realtime Cost Routing', targetQuery: 'openai batch api cost optimization' },
  { id: 'provider-arbitrage', topic: 'Multi-Provider Model Routing', targetQuery: 'llm model routing cost savings' },
...
function analyzeGaps() {
  ...
  const existingFiles = fs.readdirSync(srcResearchDir).concat(fs.readdirSync(dataArticlesDir));
  const existingArticles = existingFiles.map((f) => f.replace(/\.(njk|html|md|json)$/, ''));
